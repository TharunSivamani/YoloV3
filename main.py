...
def init(model, basic_sanity_check=True, find_max_lr=True, train=True, **kwargs):
    if basic_sanity_check:
        ...
        if find_max_lr:
            ...
            if train:
                trainer = pl.Trainer(
                    precision=16, max_epochs=cfg.NUM_EPOCHS, accelerator="gpu"
                )
                ...
                if ckpt_file:
                    checkpoint = utils.load_model_from_checkpoint(
                        cfg.DEVICE, file_name=ckpt_file
                    )
            ...
            scaled_anchors = torch.tensor(cfg.ANCHORS) * torch.tensor(cfg.S).unsqueeze(
                1
