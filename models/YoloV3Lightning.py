...
class YOLOv3LightningModel(pl.LightningModule):
    def __init__(self, in_channels=3, num_classes=20, anchors=None, S=None):
        ...
        self.anchor_list = torch.tensor(anchors) * torch.tensor(S).unsqueeze(
            1
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        loss = self.criterion(
            output, target, loss_dict=True, anchor_list=self.anchor_list
        )
    ...
    def on_validation_epoch_end(self):
        if self.metric["total_train_steps"]:
            ...
            print(
                f"Class accuracy  : {(epoch_acc['correct_class']/(epoch_acc['total_class_preds']+1e-16))*100:2f}%"
            )
            print(
                f"No obj accuracy : {(epoch_acc['correct_noobj']/(epoch_acc['total_noobj']+1e-16))*100:2f}%"
            )
            print(
